...
/// Rork Auth (Google / Apple sign-in) manager. Tokens live in the Keychain;
/// the optional account only powers cloud sync — the app stays fully usable
/// without signing in.
@Observable
final class AuthManager {
    ...
    /// Injected by Rork into UserDefaults at install time on the iOS
    /// Simulator only; absent on real devices. Computed so a Sign In tap
    ...
    private func userFromToken(_ token: String) -> User? {
        guard let payload = Self.decodePayload(token) else { return nil }
    ...
    private nonisolated struct JWTPayload: Codable, Sendable {
        let sub: String
        let email: String?
        let name: String?
        let picture: String?
    ...
    private nonisolated static func decodePayload(_ token: String) -> JWTPayload? {
    ...
    /// Refresh token: UserDefaults (simulator-injected) first, then Keychain.
    ...
    /// Refreshes the access token when it is missing or expires within
    /// 5 minutes, so long-lived Supabase sessions keep working.
    @MainActor
    func ensureValidToken() async {
        if let token = KeychainHelper.get("access_token"),
           let payload = Self.decodePayload(token),
           let exp = payload.exp,
           Date(timeIntervalSince1970: exp) > Date().addingTimeInterval(300) {
            return
        }
        await refreshToken()
    }
    ...
    /// Poll /oauth/poll-code until the host-browser popup completes,
    /// used only when the backend returned `flow: "popup"` (Rork simulator).
    ...
    /// Open the OAuth authorize URL in ASWebAuthenticationSession and resolve
    /// to the auth code from the rork-{projectID}:// callback.
...
private nonisolated struct InitiateResponse: Codable, Sendable {
    let auth_url: String
    let state: String
    let flow: String?
}

private nonisolated struct PollCodeResponse: Codable, Sendable {
    let status: String
    let code: String?
}
...
private nonisolated struct RefreshResponse: Codable, Sendable {
    let access_token: String
    let expires_in: Int
}

private nonisolated struct ErrorResponse: Codable, Sendable {
    let error: String
}
